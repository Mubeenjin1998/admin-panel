...
import AddStores from './addStore'
...
const stores = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/admin/master/store?page=${page}&search=${encodeURIComponent(debouncedSearch)}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
      console.log(response.data.stores,'================================sofl')
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      await axios.delete(`http://localhost:5000/api/admin/master/store/${productId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const getsingleProductdetails = async (productId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/admin/master/store/${productId}`, {
                ...
                <CTableHead>
                  <CTableRow>
                    <CTableHeaderCell>
                        <input
                         type="checkbox" 
                         cheked={selectedProducts.length === stores.length && stores.length > 0}
